...
<script>
import {guidanceUpdate, guidanceList, deleteGuidance} from "@/common/api";
...
export default {
  ...
  methods: {
    ...
    toAddPage(row) {
      this.$router.push({name: 'guidanceUpdate', params: {id: row.id}})
    },
    toDetailPage(row) {
      this.$router.push({name: 'guidanceDetail', params: {id: row.id}})
    },
    renderIcon() {
      return <SearchIcon/>;
    },
